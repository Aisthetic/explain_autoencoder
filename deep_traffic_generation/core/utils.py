...
def get_reconstruction(model ,
                        ...
                        dataset , 
                        timedelta : torch.Tensor=None):
    """
    Plot the reconstruction of a 1 element batch.
    :param model: The model to use for reconstruction.
    :param batch: The batch to use for reconstruction.
    :param dataset: The dataset to use for reconstruction.
    :param dataset: The timestamp to use for reconstruction.
    :return: The figure and axes of the plot.
    """
    # reconstruction
    x, info = batch
    # check if x is tensor else convert to tensor
    ...
    if "timedelta" not in model.hparams.features :
        if timedelta is None:
            raise ValueError("Timestamp is missing.")
        x = torch.cat((x, timedelta.unsqueeze(0).unsqueeze(0)), dim=1)
        x_hat = torch.cat((x_hat, timedelta.unsqueeze(0).unsqueeze(0)), dim=1)

    # if x is in cuda put in cpu
    if x.is_cuda:
        x = x.cpu()
        x_hat = x_hat.cpu()
        info = info.cpu()
    ...
    data = dataset.inverse_transform(torch.Tensor(data))
    # Later functions require the data to be in the form (batch, seq_len, features)
    # Especially the builder
    data = torch.transpose(data, 1, 2)
    data = data.reshape(data.shape[0], -1)

    # add latitude longitude info if missing
    if "latitude" not in model.hparams.features:
